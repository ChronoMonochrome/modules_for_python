...
def sqrt2(integer,power=2):
 ...
 for x in fct:
  s*=x 
 return (str(n)+'*sqrt('+str(s)+')').replace('\x1a','').replace('*sqrt(1)','')
...
def nfactors(lst):
	def test(s):exec(s)
	ERROR=0
	if type(lst)==int:lst=pack(factorization(lst))
	try:[test(i) for i in lst]
	except TypeError:
		ERROR='TypeError'
	except SyntaxError:
		ERROR='SyntaxError'
	if ERROR=='SyntaxError':return ERROR
	elif ERROR=='TypeError':
		lst=pack(lst)
	n=1
	try:
		for x in lst: n*=int(  x[x.index('**')+2:]  )+1
	except ValueError: return 'ValueError'
	return n-1

def fseek(_x,delta=10**-5):
 if type(_x)!=float: return _x
 if modf(_x)[0]<=delta: return int(_x)
 else: return _x
def decompIR(irrational):
 ...
 if denom.count('sqrt'):
   ...
   insqrt=tmpsqrt[1]
   return i1i2(compIR([denom,insqrt,ceil]),'sqrt('+tmp+')')
 return denom,insqrt,ceil

def compIR(lst):
 global rad
 if type(lst) in [str, unicode]: return lst
 if '0' in lst[1:] and lst[0]!='0': return u'0' 
 tmp=(lst[2]+u'sqrt('+lst[1]+u')/'+lst[0]).replace('sqrt'+'('+'1'+')','').replace('1'+'sqrt',u'sqrt')
...
def i1i2(i1,i2):
 ...
 i1i2ceil/=NOD
 i1i2denom/=NOD
 return normvid([str(e) for e in [i1i2denom,i1i2sqrt,i1i2ceil]])
...
def vectxvect(lst1,lst2):
  ...
  def tmpres(i1,j2,j1,i2):
   ...
   if p1[1]==p2[1]:
    ...
    return (compIR(p1)+'+'+compIR(p2)).replace('+-','-').replace('0+','').replace('+0','').replace('0-','-').replace('-0','')
  return tmpres(y1,z2,z1,y2),tmpres(x2,z1,x1,z2),tmpres(x1,y2,y1,x2)

def add(i1,i2):
 if type(i1) in [str,unicode]: i1=decompIR(i1)
 if type(i2) in [str,unicode]: i2=decompIR(i2)
 if i1[1]==i2[1]:
  insqrt='sqrt('+i1[1]+')'
  ...
  return (compIR(i1)+'+'+compIR(i2)).replace('+-','-').replace('0+','').replace('+0','').replace('0-','-').replace('-0','')
...
def normvid(x):
 ...
 elif type(x)==tuple: x=list(x)
 try:
  if eval(x[2])==0: return ['1','0','0']
  if int(x[0])<0:
   x[0]=str(int(x[0])*(-1))
   x[2]=str(int(x[2])*(-1))
 except: pass
...
def pfractions(lstfracts_str):
 ...
 for s in lstfracts_str:
  if type(s) in [str,unicode]: s=decompIR(s)
  elif type(s)==int: s=decompIR(str(s))
  elif type(s)==float:
   c=0
   while type(s)!=int:
    s*=10
